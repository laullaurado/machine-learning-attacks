...
#############################################################################
# Membership Inference Attack : Target, Shadow and Attack Model Architecture
#############################################################################
...
###############################################################################
# Attribute Inference Attack :  Target and Architecture, Attack model as above
###############################################################################
def calc_feat_linear_utk(size):
    feat = size_conv(size,3,1,1)
    feat = size_max_pool(feat,2,2)
    feat = size_conv(feat,3,1,1)
    feat = size_max_pool(feat,2,2)
...
class AttrTargetNet(nn.Module):
    def __init__(self, input_dim, hidden_layers, size, out_classes=2, n_z=32):
        super(AttrTargetNet, self).__init__()
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels=input_dim, out_channels=hidden_layers[0], kernel_size=3, padding=1),
            nn.BatchNorm2d(hidden_layers[0]),
            #nn.Dropout(p=0.2), 
        ...
        self.conv3 = nn.Sequential(
            nn.Conv2d(in_channels=hidden_layers[1], out_channels=hidden_layers[2], kernel_size=3, padding=1),
        ...
        features = calc_feat_linear_utk(size)
        self.fc = nn.Sequential(
            nn.Flatten(),
            nn.Linear((features**2 * hidden_layers[2]),n_z*2),
            nn.ReLU(inplace=True),
            #nn.Dropout(p=0.2),
            nn.Linear(n_z*2,n_z),
            nn.ReLU(inplace=True),
        ...
        self.out = nn.Linear(n_z, out_classes)
...
#Attack MLP Model
class AttackMLPAttr(nn.Module):
    def __init__(self, input_size, hidden_size=64,out_classes=2):
        super(AttackMLPAttr, self).__init__()
        self.classifier = nn.Sequential(
            nn.Linear(input_size, hidden_size),
